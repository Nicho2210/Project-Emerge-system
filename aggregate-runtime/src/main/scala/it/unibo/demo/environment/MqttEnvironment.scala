 ...
 */
class MqttEnvironment(data: Map[ID, (Position, Info)], 
                      private val neighbours: Map[ID, Set[ID]], 
                      private val emulatedData: Map[ID, Boolean])
  ...
  override def isEmulated(id: ID): Boolean =
    emulatedData.getOrElse(id, false)